...
const findPKey = async (ymd_irai, id, ymd_target) => {
  const retObj = await knex.from('hoseis').where({ ymd_irai, id_users: id, ymd_target });
  return retObj;
};
...
const update = async (inObj) => {
  const query = `update hoseis set  id_users_hosei = ${common.retValueForSql(inObj.id_users_hosei)}, ymd_hosei = ${common.retValueForSql(inObj.ymd_hosei)} where ymd_irai = '${inObj.ymd_irai}' and id_users = '${inObj.id_users}' and ymd_target = '${inObj.ymd_target}'`;
  logger.info(query);
  const retObj = await knex.raw(query);
  return retObj[0];
};
